...
function burstConfettiAt(cx, cy, count = 18) {
  for (let i = 0; i < count; i++) {
    ...
    s.style.setProperty("--rz", `${(Math.random() - 0.5) * 220}deg`);
...
function burstLettersAt(cx, cy, text, max = 12) {
  ...
  for (let i = 0; i < Math.min(max, chars.length); i++) {
    ...
    s.style.setProperty("--rz", `${(Math.random() - 0.5) * 180}deg`);
    document.body.appendChild(s);
    setTimeout(() => s.remove(), 1000);
  }
}

export function bindCopyConfetti(el, opts = {}) {
  ...
  const count = Number(opts.count || 18);
  ...
  const onClick = async (ev) => {
    ...
    if (el.dataset.cooldown === "1") return;                // anti-spam
    el.dataset.cooldown = "1"; setTimeout(() => el.removeAttribute("data-cooldown"), timeout);
...
export default function initCopyConfetti(selector = "[data-copy],[data-confetti-copy],#emailLink") {
